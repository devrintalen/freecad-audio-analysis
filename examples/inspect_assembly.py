...
def report_cavity(doc: App.Document, padding_mm: float) -> None:
    ...
    print()
    if open_to_outside:
        print("  The largest void reaches the envelope boundary: interior and exterior")
        print("  air are connected, so this model has no closed cavity as it stands.")
        print("  Cap the opening -- with the ear simulator face for a headphone, or a")
        print("  baffle for a loudspeaker -- to define the acoustic domain.")
    else:
